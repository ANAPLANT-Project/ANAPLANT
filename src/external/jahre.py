...
def main():
    ...
    # data = read_file('/Users/bennettd/dev/anaplant/static/ANAPLANT Rohdaten.xlsx')
    aufbereiten(data)
...
def plot_stadien(data_kultur: pd.DataFrame, kultur: str, element: str, path: str):
    ...
    ax.grid(alpha=.5)
    # todo: limit x axis to valid extend of this plot
    
    ax.fill_between(
        stadien,
        data_kultur["min_labor"],
        data_kultur["max_labor"],
        label=fill(f"Literatur-Zielwertbereich für {element} in {kultur} (Bergmann, 1993; Vielemeyer und Hundt, 1991)", 60),
        color="blue",
        alpha=0.15,
    )
